...
void setup()
{
  ...
  in = minim.getLineIn(Minim.STEREO, 16384);
...
void draw()
{
  ...
  for(int i = 0; i < in.bufferSize() - 1; i++)
  {
    ...
    if(in.left.get(i) >= 0.95){
      text("Threshold crossed", 5, 35);
      int j = 0;
      for(j = i; j < in.bufferSize() - 1 && j < i + 10000; j++){
        if(in.left.get(j) <= 0.95){
          /*beat = beat + 1;
          print(beat + " tap detected");*/
          beat = beat + 1;
          print("One beat ");
          break;
        }
